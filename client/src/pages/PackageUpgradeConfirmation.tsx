...
export default function PackageUpgradeConfirmation() {
  ...
  // Fetch the Package upgrade request by ID
  const {
    data: packageRequest,
    isLoading: isPackageLoading,
    error: packageError,
  } = useQuery({
    queryKey: ["listing-package-upgrade", requestId],
    queryFn: async () => {
      const res = await fetch(`/api/listing-package-upgrade/${requestId}`);
      if (!res.ok) throw new Error("Failed to fetch Package upgrade request");
      return res.json();
    },
  ...
  const listingId = packageRequest?.listing_id;
  ...
  const createdAt = packageRequest?.created_at
    ? new Date(packageRequest.created_at).toLocaleString("en-US", {
        year: "numeric",
        month: "long",
        day: "numeric",
        hour: "2-digit",
    ...
    <div className="bg-white min-h-screen py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <h3 className="text-3xl font-bold text-gray-900">Package Upgrade</h3>
            ...
            <div className="border-2 border-gray-400 rounded-md overflow-hidden">
              ...
              <div className="bg-white p-4 space-y-4">
                <div className="grid grid-cols-[200px_1fr] gap-y-2 items-start">
                  ...
                  <div className="text-neutral-800 font-bold">Upgrade From:</div>
                  <div className="text-neutral-800 font-medium">
                    {listing?.currentPackage?.package_name} to {packageRequest?.name}
                  </div>

                  <div className="text-neutral-800 font-bold">Price:</div>
                  <div className="text-neutral-800 font-medium">
                    QR{" "}
                    {packageRequest?.price != null
                      ? Number(packageRequest.price).toLocaleString("en-US")
                      : "-"}
                  </div>
