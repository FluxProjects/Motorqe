...
import { CarListing, CarMake, CarModel, CarCategory, Users } from '@shared/schema';
...
const reportSchema = z.object({
  ...
  details: z.string().min(10, 'Details must be at least 10 characters').max(500, 'Details cannot exceed 500 characters'),
});
...
const CarDetails = () => {
  ...
  const { data: sellerData, error, isLoading } = useQuery<Users>({
 ...
 const { data: makes = [] } = useQuery<CarMake[]>({
  ...
  const makeName = makes.find((m) => m.id === car?.make_id)?.name ?? 'Unknown Make';
  const modelName = models.find((m) => m.id === car?.mode_id)?.name ?? 'Unknown Model';
  const categoryName = categories.find((c) => c.id === car?.category_id)?.name ?? 'Unknown Category';
    ...
    return (
      <div className="min-h-screen bg-neutral-100 p-8">
        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-sm p-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-neutral-900 mb-2">{t('common.error')}</h1>
  ...
  const description = language === 'ar' && car.descriptionAr ? car.descriptionAr : car.description;
    ...
    <div className="bg-neutral-100 pb-16">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center space-x-2">
              <Button 
                variant="outline" 
                size="sm" 
                className={isFavorited ? "text-secondary border-secondary hover:bg-secondary-light" : ""} 
                onClick={handleFavoriteToggle}
              >
                <Heart className={`mr-1 ${isFavorited ? "fill-secondary" : ""}`} size={16} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              ...
              <div className="hidden md:grid grid-cols-5 gap-2 mt-2">
                {car.images?.slice(0, 5).map((image: string, index: number) => (
                  <div key={index} className="aspect-[16/9] bg-neutral-100 rounded-md overflow-hidden cursor-pointer">
                    <img
                      src={image}
            ...
            <div className="bg-neutral-50 p-4 rounded-lg">
              ...
              <div className="flex items-center text-neutral-600 mb-4">
                ...
                <span>{car.created_at ? formatDate(car.created_at.toString()) : 'N/A'}</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="flex flex-col items-center bg-white p-3 rounded-md">
                  ...
                  <span className="text-xs text-neutral-500">{t('car.mileage')}</span>
                </div>
                
                <div className="flex flex-col items-center bg-white p-3 rounded-md">
                  <Calendar className="text-neutral-700 mb-1" size={20} />
                  <span className="text-sm font-medium">{car.year}</span>
                  <span className="text-xs text-neutral-500">{t('car.year')}</span>
                </div>
                ...
                <div className="flex flex-col items-center bg-white p-3 rounded-md">
                  ...
                  <span className="text-xs text-neutral-500">{t('car.fuelType')}</span>
                </div>
                ...
                <div className="flex flex-col items-center bg-white p-3 rounded-md">
                  ...
                  <span className="text-xs text-neutral-500">{t('car.transmission')}</span>
                </div>
                ...
                <div className="flex flex-col items-center bg-white p-3 rounded-md">
                  ...
                  <span className="text-xs text-neutral-500">{t('car.condition')}</span>
                </div>

                {/* Color */}
                <div className="flex flex-col items-center bg-white p-3 rounded-md">
                  <Droplet className="text-neutral-700 mb-1" size={20} />
                  <span className="text-sm font-medium capitalize">{car.color}</span>
                  <span className="text-xs text-neutral-500">{t('car.color')}</span>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2">
              ...
              <CardContent className="p-0">
                <Tabs defaultValue="description" className="w-full">
                  <TabsList className="w-full grid grid-cols-2 rounded-none">
                    ...
                    <TabsTrigger value="details">{t('car.carDetails')}</TabsTrigger>
                  </TabsList>
                  ...
                  <TabsContent value="details" className="p-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                      <div className="flex justify-between">
                        <span className="text-neutral-600">{t('car.make')}</span>
                        <span className="font-medium">{makeName}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-neutral-600">{t('car.model')}</span>
                        <span className="font-medium">{modelName}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-neutral-600">{t('car.year')}</span>
                        <span className="font-medium">{car.year}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-neutral-600">{t('car.mileage')}</span>
                        <span className="font-medium">{car.mileage.toLocaleString()} mi</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-neutral-600">{t('car.fuelType')}</span>
                ...
                `car.${
                  car.fuel_type?.toLowerCase?.() || "unknownFuelType"                  
                      ...
                      <div className="flex justify-between">
                        <span className="text-neutral-600">{t('car.transmission')}</span>
                        <span className="font-medium">{t(
                `car.${
                  car.transmission?.toLowerCase?.() || "unknownTransmission"
                      ...
                      <div className="flex justify-between">
                        <span className="text-neutral-600">{t('common.category')}</span>
                        <span className="font-medium">{categoryName}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-neutral-600">{t('car.listingDate')}</span>
                        <span className="font-medium">{car.created_at ? formatDate(car.created_at.toString()) : 'N/A'}</span>
            ...
            {similarCars && similarCars?.length > 0 && (
              <div className="mt-8">
                <h2 className="text-xl font-bold mb-4">{t('car.similarCars')}</h2>
          ...
          <div>
            ...
            <Card>
              <CardContent className="p-6">
                <h2 className="text-lg font-semibold mb-4">{t('common.sellerInfo')}</h2>
                <div className="flex items-center mb-4">
                  <Avatar className="h-16 w-16 mr-4">
                  <AvatarImage src={sellerData?.avatar ?? undefined} alt={sellerData?.username ?? ''} />
                    <AvatarFallback>{sellerData?.username.charAt(0).toUpperCase()}</AvatarFallback>
                  </Avatar>
                  <div>
                    <p className="font-medium text-lg">{sellerData?.username}</p>
                    <p className="text-sm text-neutral-500">{t('common.memberSince')}{' '}
                    {sellerData?.created_at ? new Date(sellerData.created_at).getFullYear() : ''}</p>
                    {sellerData?.is_email_verified && (
                      <Badge variant="outline" className="mt-1">
                        <Check size={12} className="mr-1" /> {t('common.verified')}
                      </Badge>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-col space-y-2">
                  <div>
                    <span className="text-sm text-neutral-500">{t('common.totalListings')}:</span>
                    <span className="font-medium ml-2">{sellerData?.listingCount || 0}</span>
                  </div>
                  <div>
                    <span className="text-sm text-neutral-500">{t('common.responseRate')}:</span>
                    <span className="font-medium ml-2">{sellerData?.responseRate || 0}%</span>
                  </div>
                  <div>
                    <span className="text-sm text-neutral-500">{t('common.avgResponseTime')}:</span>
                    <span className="font-medium ml-2">{sellerData?.responseTime || 0} {t('common.hours')}</span>
                  </div>
                </div>
      ...
      <Dialog open={contactDialogOpen} onOpenChange={setContactDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {t('car.contactSellerDesc')}
            </DialogDescription>
          </DialogHeader>
          
          <Form {...messageForm}>
            <form onSubmit={messageForm.handleSubmit(handleContactSeller)} className="space-y-4">
      ...
      <Dialog open={reportDialogOpen} onOpenChange={setReportDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <Form {...reportForm}>
            <form onSubmit={reportForm.handleSubmit(handleReportListing)} className="space-y-4">
              <div className="bg-neutral-50 p-3 rounded-md text-sm mb-4">
                <p className="font-medium">
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="fraud">{t('common.reportReasonFraud')}</SelectItem>
                        <SelectItem value="inappropriate">{t('common.reportReasonInappropriate')}</SelectItem>
                        <SelectItem value="duplicate">{t('common.reportReasonDuplicate')}</SelectItem>
                        <SelectItem value="misrepresentation">{t('common.reportReasonMisrepresentation')}</SelectItem>
                        <SelectItem value="other">{t('common.reportReasonOther')}</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>
