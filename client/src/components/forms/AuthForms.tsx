...
import { useAuth } from '@/contexts/AuthContext'; // Import the custom AuthContext
...
const registerFormSchema = z.object({
  ...
  username: z.string().min(3).regex(/^[a-zA-Z0-9_]+$/, 'Only letters, numbers and underscores'),
...
export const AuthForms = ({ initialView, onClose, onSwitchView }: AuthFormsProps) => {
  const { login } = useAuth(); // Your custom authentication hook
  ...
  const registerForm = useForm<RegisterFormValues>({
    ...
    defaultValues: {
      ...
      password: '',
      confirmPassword: '',
      role: 'SELLER', // or 'seller' or 'showroom' based on your default
      roleId: 2,
      termsAgreement: true,
    },
  });

  const handleApiError = (error: unknown, form: typeof loginForm | typeof registerForm) => {
  ...
  const onLoginSubmit = async (values: LoginFormValues) => {
    ...
    if (!loginForm.formState.isValid) {
      console.warn("⛔ Submission blocked — form is invalid", registerForm.formState.errors);
      return;
    }
    ...
    try {
      ...
      if (data.user.roleId === 1) {
        ...
        console.log('❓ Unknown role, navigating to home (roleId:', data.user.roleId, ')');
        navigate('/');
      }
  ...
  console.log("register form state: ",registerForm.formState); // Check for `disabled: true`
  console.log("registerForm form state errors: ", registerForm.formState.errors); // Log validation errors to check which fields are failing
  ...
  return (
    <div className="p-6">
                ...
                <FormItem className="mb-6">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="space-y-1 leading-none">
                      <Label htmlFor="termsAgreement" className="text-sm text-neutral-600">
                        {t('auth.agreeToTerms')}
                      </Label>
                    </div>
                  </div>
      ...
      <div className="mt-6 text-center text-sm text-neutral-600">
        {view === 'login' ? (
          <>
            {t('auth.dontHaveAccount')} <Button variant="link" className="p-0 text-primary hover:text-primary/90 font-medium" onClick={() => switchView('register')}>{t('auth.registerNow')}</Button>
          </>
        ) : (
          <>
            {t('auth.alreadyHaveAccount')} <Button variant="link" className="p-0 text-primary hover:text-primary/90 font-medium" onClick={() => switchView('login')}>{t('auth.login')}</Button>
          </>
        )}
      </div>
    </div>
