...
export const CarListingActionDialog = ({
  ...
  const { user } = useAuth();
  const [unfeature, setUnfeature] = useState(false);
  ...
  const hasActionPermission = () => {
    ...
    switch (actionType) {
      ...
      case "reject":
        return (
          hasPermission(roleName, Permission.APPROVE_LISTINGS) ||
      ...
      case "feature":
        ...
        console.log(
          "Has Manage All Listings Permission:",
          hasPermission(roleName, Permission.MANAGE_ALL_LISTINGS)
        );
        console.log(
          "Has Create Promotions Permission:",
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="unfeature"
                  checked={unfeature}
                  onCheckedChange={(checked) => setUnfeature(!!checked)}
                  className="border-slate-500 data-[state=checked]:bg-blue-600 data-[state=checked]:border-blue-600"
                />
                <Label htmlFor="unfeature" className="text-slate-300">
                  {listing.is_featured === "true" && unfeature
        ...
        <DialogFooter>
          ...
          >
            {actionInProgress ? (
              ...
              <>
                {actionType === "publish" && <Check className="h-4 w-4 mr-2" />}
                {actionType === "approve" && <Check className="h-4 w-4 mr-2" />}
                {actionType === "reject" && <X className="h-4 w-4 mr-2" />}
                {actionType === "sold" && <Check className="h-4 w-4 mr-2" />}
