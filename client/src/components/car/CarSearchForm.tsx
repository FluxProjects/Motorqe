...
import { PriceInputCombobox } from "../ui/priceinput";
...
const CarSearchForm = ({ is_garage }: CarSearchFormProps) => {
  ...
  const showField = (fieldName: keyof SearchFormValues) => {
    if (activeTab === "all") {
      return !["condition", "keyword", "location", "category"].includes(
        fieldName
      );
    }
    if (activeTab === "new") {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div
              className={`w-full flex flex-wrap gap-4 ${
                showAdvanced ? "justify-start" : "justify-center"
              }`}
            >
              {/* Make */}
